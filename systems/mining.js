...
function resolveRock(state, rockOrId){
  if (!rockOrId) {
    return ROCKS.find(r=>r.id===state.selectedRockId) || ROCKS[0];
  }
  if (typeof rockOrId === 'string') {
    return ROCKS.find(r=>r.id===rockOrId) || null;
  }
  if (rockOrId && rockOrId.id) {
    return ROCKS.find(r=>r.id===rockOrId.id) || rockOrId;
  }
  return null;
}
...
function pickSpeedFromState(state){
  ...
  const def = pickId ? ITEMS[pickId] : null;
  return (def?.speed) || 1;
}
...
export function canMine(state, rockOrId){
  ...
  if (lvl < requiredLevel(rock)) return false;
  return true;
}
...
export function startMine(state, rockOrId, onDone){
  ...
  // Keep selection in sync with UI
